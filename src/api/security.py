...
from api.constants import PROJECT_DIRECTORY
...
def _build_cipher_from_value(value: str | bytes, source: str) -> Fernet:
    key_bytes = value.encode(ENCODING) if isinstance(value, str) else value
    try:
...
def _load_cipher() -> Fernet:
    ...
    if TOKEN_KEY_FILE.exists():
        token_value = TOKEN_KEY_FILE.read_text(encoding=ENCODING).strip()
        return _build_cipher_from_value(token_value, str(TOKEN_KEY_FILE))

    key = Fernet.generate_key()
    TOKEN_KEY_FILE.write_text(key.decode(ENCODING), encoding=ENCODING)
    try:
        if os.name != "nt":
...
def encrypt_factorio_token(value: str) -> bytes:
    ...
    return _TOKEN_CIPHER.encrypt(value.encode(ENCODING))


def decrypt_factorio_token(value: bytes) -> str:
    """Decrypt a stored Factorio authentication token."""
    return _TOKEN_CIPHER.decrypt(value).decode(ENCODING)
